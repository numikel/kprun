...
fn run() -> Result<()> {
    ...
    let master = read_master_from_keystore(&cfg.db_path).map_err(|e| match e {
    ...
    // Warn and audit before printing, mirroring `get --reveal`; an
    // audit-write failure fails the command.
    warn_secret_display();
    audit_access(
        &cfg,
        AuditRecord::new(
